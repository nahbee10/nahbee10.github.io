...
function setup() {
  ...
  var vec_att_next = new Vec2D(vec_att.x+270, vec_att.y);
  var vec_att2_next = new Vec2D(vec_att2.x+270, vec_att2.y);
  first_class_blob = new Bblob(d,colors_b[c_index], colors_b_t_t[c_index], x_el, y_el, x_el_t, y_el_t, vec_att, vec_att2, 0);
  first_class_blob2 = new Bblob(d,colors_b[c_index], colors_b_t_t[c_index], x_el, y_el, x_el_t, y_el_t, vec_att_next, vec_att2_next, 270);
...
function Bblob(which_Degas, color, t_color, x_el, y_el, x_el_t, y_el_t, fir_att, sec_att, x_cord) {
  ...
  this.updateBlobs = function(){
    ...
    this.x_changed = [this.fifth_x, (this.fourth_x+this.fifth_x)/2, this.fourth_x];
    this.y_changed = [this.fifth_y, (this.fourth_y+this.fifth_y)/2+20, this.fourth_y];
